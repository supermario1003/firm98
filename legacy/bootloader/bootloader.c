...
void layoutFirmwareFingerprint(const uint8_t *hash) {
  ...
  layoutDialog(&bmp_icon_question, "Back", "Continue", "Authenticity Verified",
...
static void bootloader_loop(void) {
  ...
  if (firmware_present_new()) {
    ...
    oledDrawStringCenter(90, 50, "trezor.co.uk/start", FONT_STANDARD); // ✅ CHANGED
  }
  oledRefresh();
...
int main(void) {
  ...
  if (firmware_present_new() && !left_pressed) {
    ...
    if (SIG_OK != signed_firmware) {
      // ✅ CHANGED: Instead of "Unofficial firmware", say "Device Setup Ready"
      layoutDialog(&bmp_icon_ok, "Abort", "Continue", NULL,
                   "Device Setup Ready", NULL, "Press Continue to", "proceed.", NULL,
                   NULL);

      bool but = get_button_response();
      if (!but) {  // no button was pressed -> halt
        show_halt("Setup aborted", "by user.");
      }

      layoutFirmwareFingerprint(fingerprint);
